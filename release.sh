...
# Check if mkdocs.yml exists and mkdocs is in PATH
if [ -f "mkdocs.yml" ]; then
    if ! command -v mkdocs >/dev/null 2>&1; then
        echo "Error: mkdocs.yml found but mkdocs is not in PATH."
        exit 1
    fi
fi
...
# Build the documentation using mkdocs only if mkdocs.yml exists
if [ -f "mkdocs.yml" ]; then
    mkdocs build --clean
    if [ $? -ne 0 ]; then
    ...
    echo "Skipping mkdocs build: mkdocs.yml not found."
fi
...
if [ -f "mkdocs.yml" ]; then
    ...
    echo "Skipping mkdocs deploy: mkdocs.yml not found."
fi
